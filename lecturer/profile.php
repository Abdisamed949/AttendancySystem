<?php
/**
 * Profile & Password (Lecturer only) — edit own Full Name/Email, change
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_profile') {
        ...
        if ($validationError === '') {
            ...
            // Keep lecturers.full_name in sync — other admin pages (e.g.
            // admin/courses.php's lecturer dropdown) read it from there.
            $updateLecturerStmt = $conn->prepare('UPDATE lecturers SET full_name = ? WHERE user_id = ?');
            $updateLecturerStmt->bind_param('si', $fullName, $currentUserId);
